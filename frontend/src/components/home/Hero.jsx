...
const Hero = ({ cars, carsdelevered }) => {
  ...
  return (
    <section className="relative bg-slate-900 text-white py-12 sm:py-20 px-4 rounded-2xl sm:rounded-3xl mx-2 sm:mx-4 mt-4 overflow-hidden shadow-2xl">
      {/* خلفية جمالية */}
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center space-y-8">
        {/* شارات الثقة */}
        <div className="flex flex-wrap justify-center gap-2 sm:gap-4 text-[10px] sm:text-xs font-medium text-slate-300 mb-4">
        ...
        <div className="pt-4 text-xs sm:text-sm text-slate-400 flex flex-wrap items-center justify-center gap-1">
          <span>إحصائيات مباشرة: </span>
          <span className="text-white font-bold">{cars?.length}+ سيارة موثقة</span>
          <span> • </span>
          <span className="text-white font-bold">{carsdelevered?.length} استعادة ناجحة</span>
        </div>
      </div>
    </section>
  );
